...
AnalogBar::AnalogBar(Panel *parent, const char *panelName) : Panel(parent, panelName)
{
	...
	m_fHomeValue = 2.0f;
	m_HomeColor = static_cast<Panel*>( this )->GetFgColor();
}
...
void AnalogBar::Paint()
{
	...
	for (int i = 0; i < segmentsDrawn; i++)
		PaintSegment( x, y, tall, wide, static_cast<Panel*>( this )->GetFgColor(), i == iHomeIndex );

	for (int i = segmentsDrawn; i < segmentTotal; i++)
		PaintSegment( x, y, tall, wide, static_cast<Panel*>( this )->GetBgColor(), i == iHomeIndex );
}
...
void AnalogBar::SetAnalogValue(float analogValue)
{
	if (analogValue != _analogValue)
	{
		...
		static_cast<Panel*>( this )->Repaint();
	}
}
...
{
	static_cast<Panel*>( this )->ApplySchemeSettings( pScheme );

	SetBgColor( Color(
		255 - static_cast<Panel*>( this )->GetFgColor().r(),
		255 - static_cast<Panel*>( this )->GetFgColor().g(),
		255 - static_cast<Panel*>( this )->GetFgColor().b(),
		static_cast<Panel*>( this )->GetFgColor().a()
	) );
}
...
void AnalogBar::ApplySettings(KeyValues *inResourceData)
{
	...
	static_cast<Panel*>( this )->ApplySettings( inResourceData );
}
...
{
	static_cast<Panel*>( this )->GetSettings( outResourceData );
...
const char *AnalogBar::GetDescription( void )
{
	...
	_snprintf(buf, sizeof(buf), "%s, string analogValue, string variable", static_cast<Panel*>( this )->GetDescription() );
	return buf;
}
...
void ContinuousAnalogBar::Paint()
{
	...
	surface()->DrawSetColor( static_cast<Panel*>( this )->GetFgColor() );
