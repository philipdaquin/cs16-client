...
void CXashFileSystem::RemoveFile(const char *pRelativePath, const char *pathID)
{
    ...
    char localPath[4096];
    if( GetLocalPath( pRelativePath, localPath, sizeof( localPath ) ) )
        unlink( localPath );
}
...
{
    return gFileSystemAPI.FS_FileSize( pFileName, false ) >= 0;
}
...
bool CXashFileSystem::IsDirectory(const char *pFileName)
{
    struct stat buf;
    char localPath[4096];
    const char *path = GetLocalPath( pFileName, localPath, sizeof( localPath ) );
...
{
    if( !pFullpath || !pRelative || !pFullpath[0] )
    {
        pRelative[0] = 0;
        return false;
    }

    Q_strncpy( pRelative, pFullpath, MAX_PATH );
    FixSlashes( pRelative );
    return false;
}
