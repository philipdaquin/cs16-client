...
//          of the horizontal and vertical ratios
//-----------------------------------------------------------------------------
int GetAlternateProportionalValueFromScaled(vgui2::HScheme hScheme, int scaledValue)
{
...
//-----------------------------------------------------------------------------
void ApplyBackgroundSchemeSettings(EditablePanel *pWindow, vgui2::IScheme *pScheme)
{
	if (!pWindow || !pScheme)
		return;
...
static void FixupBackgroundPanels(EditablePanel *pWindow, int offsetX, int offsetY)
{
	...
	{
		pPanel->SetZPos(-19);
		pPanel->SetBounds(offsetX + captionInset, offsetY + inset + logoInset, screenWide, logoSize);
	}
}
//-----------------------------------------------------------------------------
